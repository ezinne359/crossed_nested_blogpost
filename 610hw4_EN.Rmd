...
winter_clothes <- winter_clothes %>% mutate(price = case_when(product == "boots" ~ price  , product == "jacket" ~ price + 10, product == "gloves" ~ price +4) )
...
Now, let's look at some real data. We will be looking at policing data in the US from 17 different states: 
...
map1 <- ggplot() +
  ...
  viridis::scale_fill_viridis(name="Total Stops", direction=-1, labels = scales::unit_format(unit = "K", scale = 1e-3)) +
  coord_map("polyconic") +
  ggthemes::theme_map() +
